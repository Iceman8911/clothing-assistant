...
export default function CreateClothingModal(
	...
	const sanitiseNumberInput = (e: FocusEvent) => {
		...
		if (targetValue > MAX_NUMBER_INPUT) {
			target.value = `${MAX_NUMBER_INPUT}`;
		} else if (targetValue < MIN_NUMBER_INPUT) {
			target.value = `${MIN_NUMBER_INPUT}`;
		}
	};
	...
	const isEditMode = () => !!prop.clothIdToEdit;
	...
	createEffect(
		on([isEditMode, prop.stateAccessor], async () => {
			if (!prop.stateAccessor() || !prop.clothIdToEdit) return;
			...
			if (isEditMode()) {
				for (const k in clothingData) {
					if (Object.hasOwn(clothingData, k)) {
					...
					>
						<Suspense>
							<div
								...
								onClick={clothingDisplayClick}
								onKeyUp={clothingDisplayClick}
							>
								<Show
									when={
										clothingItemBase64Url() &&
										...
										onClick={async (e) => {
											...
											try {
												...
												for (const i in aiJsonResponse) {
													if (Object.hasOwn(aiJsonResponse, i)) {
						...
						<fieldset class="fieldset row-start-3 col-span-4">
							...
							/>
							<label class="label" for="clothing-img-input">
								Max size 10MB
							</label>
						</fieldset>
